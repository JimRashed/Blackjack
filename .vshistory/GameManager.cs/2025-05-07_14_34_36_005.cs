        ...
        private void PlayPlayerTurns()
        {
            ...
            for (int currentPlayerNumber = 0; currentPlayerNumber<playerCount; currentPlayerNumber++)
            {
                ...
                if (currentPlayer.Playing)
                {
                    ...
                    } while (!currentPlayer.Playing || userChoice != Constants.STAND && !GameRules.Bust(currentPlayer.Hand)); //Menu keeps printing while player does not stand or does not bust
                }
                ClearAndDisplay();
            }
        }
