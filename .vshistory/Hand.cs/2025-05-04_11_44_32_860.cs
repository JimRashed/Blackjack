...
namespace Rashed_Blackjack
{
    public class Hand
    {
        ...
        public Hand(string[] suitPriorities)
        {
            this.suitPriorities = suitPriorities;
            hand = new List<Card>();
        ...
        public void Sort(bool descending = false)
        {
            ...
            if (!descending)
            {
                for (int outerLoop = 0; outerLoop < _hand.Count; outerLoop++)
                {
                    lowestIndex = outerLoop;

                    for (int innerLoop = outerLoop + 1; innerLoop < _hand.Count; innerLoop++)
                    {
                        if (_hand[innerLoop] < _hand[lowestIndex])
                        {
                            lowestIndex = innerLoop;
                        }
                    }
                    tempStorage = _hand[outerLoop];
                    _hand[outerLoop] = _hand[lowestIndex];
                    _hand[lowestIndex] = tempStorage;
                }
            }
            else
            {
                for (int outerLoop = 0; outerLoop < _hand.Count; outerLoop++)
                {
                    ...
                    for (int innerLoop = outerLoop + 1; innerLoop < _hand.Count; innerLoop++)
                    {
                        if (_hand[innerLoop] > _hand[highestIndex])
                        {
                            highestIndex = innerLoop;
                        }
                    }
                    tempStorage = _hand[outerLoop];
                    _hand[outerLoop] = _hand[highestIndex];
                    _hand[highestIndex] = tempStorage;
                }
            }
