...
*/
    public class Deck
    {
        ...
        public Deck()
        {
            cardList = new List<Card>(); //initializes an empty deck
        }

        public Deck(bool hasJokers)
        {
            cardList = new List<Card>(); //Initializes an empty deck to be filled.
            if (!hasJokers)
            {
                for (int i = 1; i <= Globals.NORMALDECKSIZE; i++) //Ensures that 52 different cards are created in order.  1 to 52.
                {
            ...
            {
                for (int i = 1; i <= Globals.JOKERDECKSIZE; i++) //Creates 54 different cards in order. 1 to 54
                {
                    Card tempCard = new Card(i);
                    cardList.Add(tempCard);
                }
        ...
        public Card Draw()
        {
            Card tempCard = (Card)cardList[cardList.Count - 1]; //Stores the top card in the deck in a temporary variable
